...
<template>
  ...
  >
    <img
      :src="$store.state.bookDetails.image"
      alt="Book Example Image"
      class="basis-2/6 h-full md:w-full"
    />
    <div class="basis-4/6 pl-8 md:pl-0 md:w-full md:grow">
      <div class="text-4xl font-medium">
        {{ $store.state.bookDetails.title }}
      </div>
      <div class="text-xl pb-4">
        {{ $store.state.bookDetails.subtitle }}
      </div>
      <BookDetailsComponent>
        <template #detail-info> Authors </template>
        <template #detail-info-value>
          {{ $store.state.bookDetails.authors }}
        </template>
      </BookDetailsComponent>
      <BookDetailsComponent>
        <template #detail-info> Publisher </template>
        <template #detail-info-value>
          {{ $store.state.bookDetails.publisher }}
        </template>
      </BookDetailsComponent>
      <BookDetailsComponent>
        <template #detail-info> Pages </template>
        <template #detail-info-value>
          {{ $store.state.bookDetails.pages }}
        </template>
      </BookDetailsComponent>
      <BookDetailsComponent>
        <template #detail-info> Year </template>
        <template #detail-info-value>
          {{ $store.state.bookDetails.year }}
        </template>
      </BookDetailsComponent>
      <div class="py-4">
        <p class="font-medium">Description</p>
        <p class="text-justify text-sm">
          {{ $store.state.bookDetails.description }}
        </p>
      </div>
